...
function getFontStyle(font: Font): FontStyles {
    ...
    if (font.isMono()) style |= FontStyles.MonsopacedFont;
...
function buildViewLines(structuredText: StructuredText, pageWidth: number): ViewLine[] {
    ...
    function pushFontStyle() {
        ...
        if (prevStyle !== undefined) {
            prevStyle.end = start;
            if (prevStyle.end - prevStyle.start <= 0) fontStyleSpans!.pop();
        }
        fontStyleSpans!.push({
            start,
            end: -1,
    ...
    structuredText.walk({
        ...
        endLine() {
            ...
            while (true) {
                ...
                if (lastSpan === undefined) break;
                lastSpan.end = text.length;
                if (lastSpan.end - lastSpan.start > 0) break;
                fontStyleSpans!.pop();
            }
    ...
    return viewLines.filter(line => line.text.trim() !== '').sort((a, b) => a.y - b.y);
}
...
function buildViewBlocks(viewLines: ViewLine[], page: number): ViewBlock[] {
    ...
    for (let i = 1; i < viewLines.length; i++) {
        ...
        const continueCurrentBlock = verticalGap < prev.height * 1.2 && !isTextCue(curr);
...
function convertBlock(currentBlock: ViewLine[], page: number): ViewBlock {
    ...
    function getKind(): ViewBlockKind {
        if (isTextCue({ text, fontStyleSpans })) return 'cue';
        const span = getLongestSpan(fontStyleSpans);
        if (span.styles & FontStyles.Italic) return 'info';
        if (span.styles & FontStyles.Bold && text.match(/akt|szene/i)) return 'division';
        return 'unknown';
    }
...
function buildMarkdown({ text, fontStyleSpans }: TextWithFormat): string {
    const output = [];
...
function ViewBlockMenu({
    ...
    function ignore(what: 'this' | 'all' = 'this') {
        if (what === 'this') {
            ...
            for (let i = 0; i < pageContext.numPages; i++) {
                ...
                const ignoredBlocks = blocks.filter(
                    block => block.text.trim() === viewBlock.text.trim(),
                );
    ...
    function connect(where: 'upward' | 'downward') {
        ...
        if (where === 'upward' && viewBlock.backwardLink === undefined && viewBlock.page >= 1) {
            ...
            if (viewBlock.kind === 'unknown') viewBlock.kind = 'connected';
            if (previousBlock.kind === 'unknown') previousBlock.kind = 'connected';
            pageContext.invalidatePage(viewBlock.page - 1);
        } else if (
            where === 'downward' &&
            viewBlock.forwardLink === undefined &&
            viewBlock.page < pageContext.numPages - 1
        ) {
            const nextBlock =
            ...
            if (viewBlock.kind === 'unknown') viewBlock.kind = 'connected';
            if (nextBlock.kind === 'unknown') nextBlock.kind = 'connected';
            pageContext.invalidatePage(viewBlock.page + 1);
        }
        pageContext.invalidatePage(viewBlock.page);
    }
    ...
    function disconnect(where: 'upward' | 'downward') {
        if (where === 'upward' && viewBlock.backwardLink !== undefined) {
            ...
            if (viewBlock.kind === 'connected' && !connected(viewBlock)) viewBlock.kind = 'unknown';
            ...
            viewBlock.forwardLink = undefined;
            if (viewBlock.kind === 'connected' && !connected(viewBlock)) viewBlock.kind = 'unknown';
            if (nextBlock.kind === 'connected' && !connected(nextBlock)) nextBlock.kind = 'unknown';
            pageContext.invalidatePage(viewBlock.page + 1);
        }
        pageContext.invalidatePage(viewBlock.page);
    }
    ...
    const renderedOptions = createMemo(() => {
        if (viewBlock.kind === 'unknown') {
            ...
            return <li onClick={() => ignore()}>Kein Abschnitt</li>;
        }
        return <></>;
    });
...
function DistributionDialog(props: {
    ...
    function commit() {
        ...
        if (result.length === 0) props.closer(undefined);
        else props.closer(result);
    }
        ...
        <>
            ...
            <div class="actors-selection">
                {Array.from(props.actors.keys())
                    ...
                    .map(actor => (
                        <ActorPill onClick={toggleSelection} data-actor={actor} children={actor} />
                    ))}
            </div>
            <div class="bottom-line">
                <button class="primary-button" onClick={commit} disabled={selected().length === 0}>
                    Bestätigen
                </button>
            </div>
        </>
    ...
    const canvas = (
        <canvas
            ...
            style={{
                ...
                height: `${Math.floor(pageHeight * scaleFactor)}px`,
            }}
        />
    ...
    for (const block of viewBlocks) {
        const visualBlock = (
            <span
                ...
                <i class="menu-icon" onClick={onOpenMenu}>
                    &#xF5D3;
                </i>
            </span>
        ) as HTMLSpanElement;

        installPopoverMenuHandler(visualBlock, 'bottom-start', ViewBlockMenu, {
            viewBlock: block,
        ...
        if (currentDivision === undefined && block.knownDivision !== undefined) {
            ...
            if (
                ...
                const connection = viewBlocks.includes(lastChild)
                    ? lastChild.y +
                      Math.max(0, Math.sign(lastChild.height - fontSize / 2)) * lastChild.height
                    : pageHeight;

                const path = (
                    <path
                        d={`M${pageMarginLeft * ONE_THIRD},${0} v${connection}`}
                        stroke-width="1"
                        stroke="#808080"
...
function createPageRenderer(mupdf: MupdfLib): PageRenderer {
    ...
    return {
        render(page) {
            ...
            const pixmapScale = mupdf.Matrix.scale(
            ...
            renderedPage.getContext('2d')!.putImageData(imageData, 0, 0);
...
function getCue(block: ViewBlock): ViewBlock {
    if (block.kind === 'cue' || block.backwardLink === undefined) return block;
...
function populateDocumentInfo(
    ...
    for (const block of unpackViewBlocks(allPageInfo)) {
        if (block.kind === 'unknown') unknowns.push(block);
...
function buildConnectedMarkdown(block: ViewBlock): string {
    ...
    while (start.backwardLink !== undefined) start = start.backwardLink;
...
function buildSemiQuiptCueData(
    ...
    for (const block of unpackViewBlocks(allPageInfo)) {
        switch (block.kind) {
            case 'division':
                {
                    if (currentDivision.textCues.length > 0) divisions.push(currentDivision);
                    currentDivision = {
                        name: block.text,
                        textCues: [],
                    };
            ...
            case 'cue':
                {
                    ...
                    if (block.backwardLink !== undefined) {
                        const actor2 =
                            getCue(block.backwardLink).text.match(actorsRegex)?.[0].trim() ?? '';
                        if (actor === actor2) continue;
                    }

                    let actors: string[];
                    if (actorsMap.has(actor)) actors = [actor];
                    else {
                        const mappedActors = actorsMapping.get(actor);
                        if (mappedActors === undefined) throw 'unexpected undefined actors mapping';
...
function FinalizeScriptView(props: {
    ...
    function onScroll() {
        ...
        while (currentElement !== null) {
            if (
                currentElement.classList.contains('script-divsion') &&
    ...
    const activeDivisions = createMemo(() => {
        ...
        for (const semiDivision of props.semiDivisions) {
            ...
            if (division.textCues.length > 0) result.push(division);
        }
        return result;
    });
...
export function DocumentView(props: {
    ...
    function onScroll() {
        ...
        while (currentElement !== null) {
            if (
                currentElement.classList.contains('full-page') &&
                currentElement instanceof HTMLElement
            ) {
    ...
    const [actorsContext, updateActorsMapping] = createActorsContext(actors, actorsMapping, () =>
        setSignal({}),
    );

    const pageInfos = createMemo(() => {
        const pageInfos = analyzePagesWithOptions(rawPageInfoCache, allPages, {
            header: header(),
            footer: footer(),
        });
    ...
    const pageInvalidatables = allPages.map((page, index) =>
        createInvalidatable(() => renderPage(page, index)),
    );
    ...
    const pages = createMemo(() => {
        return pageInvalidatables.map(([page]) => page());
    });
    ...
    const renderedWarnings = createMemo(() => {
        ...
        return Array.from(pageMap.entries()).map(([page, count]) => (
            <li data-page={page}>
                <i class="bi bi-exclamation-triangle-fill" style={{ color: 'orange' }} /> Seite{' '}
                {page + 1}: {pluralize(count, 'Warnung', 'Warnungen')}
            </li>
        ));
    });
    ...
    function ensureNumberInput(event: KeyboardEvent) {
        if (!allowedNumberKeys.has(event.key)) event.preventDefault();
    }
    ...
    function renderActor([actor, count]: [string, number]) {
        let actorPill = (
            <ActorPill onClick={toggleMenu} children={actor} extra={`(${count})`} />
        ) as HTMLSpanElement;
        installPopoverMenuHandler(actorPill, 'top', ActorMenu, {
            actor,
            actorsContext,
        });
        return actorPill;
                        ...
                        <ul onClick={gotoDivision}>
                            {signal().x ??
                                divisions.map(division => (
                                    <li data-page={division.page}>{division.text}</li>
                                ))}
                        </ul>
                    </section>
                    {(signal().x ?? unknowns.length) === 0 ? null : (
                        <>
                            ...
                            <section class="warnings">
                                <ul onClick={gotoDivision}>{renderedWarnings()}</ul>
                            </section>
                        </>
                    )}
                </div>
