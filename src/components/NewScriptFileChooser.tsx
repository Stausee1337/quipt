...
function createPageRenderer(mupdf: MupdfLib, doc: PDFDocument): PageContext {
    ...
    function buildScaleFactorFromSize(
    ...
    return {
        renderAtSize(pageIndex, size) {
            ...
            const pixmapScale = mupdf.Matrix.scale(
                scaleFactorX * pixelRatio,
                scaleFactorY * pixelRatio,
            );
...
function mouseEnter(this: Owner, event: MouseEvent & { currentTarget: HTMLSpanElement }) {
    ...
    createRoot(dispose => {
        ...
        [img.width, img.height, img.src] = pageContext.renderAtSize(
            Number(reference.dataset.page),
            { height },
        );
        ...
        onMount(() => {
            popper = createPopper(reference, popoverMenu, { placement: 'top' });
...
function PDFPageSelector(props: {
    filename: string;
    mupdf: MupdfLib;
    doc: PDFDocument;
    ...
    const images = Array.from({ length: doc.countPages() }).map(
        () => (<img />) as HTMLImageElement,
    );
    const thumbnails = images.map((img, idx) => (
        <div class="thumbnail" data-page={idx} onMouseEnter={mouseEnter.bind(owner)}>
            {img}
    ...
    function renderOneThumbnail(pageIndex: number) {
        if (pageIndex + 1 < images.length) setTimeout(() => renderOneThumbnail(pageIndex + 1));
    ...
    function updateScrollable() {
        ...
        setCanScrollRight(
            horizontalThumbnails.scrollLeft <
                horizontalThumbnails.scrollWidth - horizontalThumbnails.offsetWidth,
        );
    }
    ...
    function scrollThumbnails(dir: 'left' | 'right') {
        const dx = horizontalThumbnails.offsetWidth * 0.8 * (-1) ** Number(dir === 'left');
    ...
    return (
        <>
            ...
            />
            <p>
                Entferne Seiten, die nicht teil des Seiten die nicht teil des Skripts sind. Achte
                insbesondere auf Seiten am Ende des Dokuments
            </p>
            <div class="page-selector">
                <i class="bi bi-chevron-left" onClick={() => scrollThumbnails('left')} />
                <i class="bi bi-chevron-right" onClick={() => scrollThumbnails('right')} />
                <div
                    ref={horizontalThumbnails}
                    class="horizontal-thumbnails"
                    classList={{
                        'scroll-left': canScrollLeft(),
                        'scroll-right': canScrollRight(),
                    }}
...
function FileUpload(props: {
    ...
    function drop(event: DragEvent) {
        ...
        if (dataTransfer !== null && dataTransfer.files.length > 0) loadFile(dataTransfer.files[0]);
    }

    function inputChange(event: Event) {
        const target = event.target as HTMLInputElement;
        if (target.files !== null && target.files.length > 0) loadFile(target.files[0]);
    }

    async function loadFile(file: File) {
        if (file.type !== 'application/pdf') setError(true);
    ...
    return (
        <>
            <label
                ...
                onDrop={drop}>
                <input type="file" accept="application/pdf" onChange={inputChange} />
                <UploadIcon />
                <p>
                    Klicke order ziehe eine Datei auf diese Fläche, um sie in den PDF Editor zu
                    laden
                </p>
            </label>
